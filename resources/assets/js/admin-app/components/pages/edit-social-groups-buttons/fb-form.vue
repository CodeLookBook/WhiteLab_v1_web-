...
<template lang="pug">
    ...
    el-form(:model="inputs", :rules="rules", label-position="left", ref="form")
        ...
        el-form-item(v-if="LANGUAGES.RUSSIAN === language", label="Ссылка на группу в Facebook:", prop="reference")
            el-input(v-model="inputs.reference", :placeholder="inputs.placeholder", :value="inputs.reference", v-cloak)
        el-form-item(v-if="LANGUAGES.SLOVAK  === language", label="Odkaz ku skupine na Facebooku:", prop="reference")
            el-input(v-model="inputs.reference", :placeholder="inputs.placeholder", :value="inputs.reference", v-cloak)
    ...
    export default {
        ...
        methods: {
            ...mapActions('SocialGroups', [
                ...
                'uploadFacebookGroupReference',
                'deleteFacebookGroupReference',
            ]),
            ...
            onSaveButtonClick() {
                this.$refs['form'].validate((valid) => {
                    ...
                    if (valid) {
                        ...
                        this.uploadFacebookGroupReference(this.inputs.reference).then(
                            (success) => {
            ...
            onDeleteButtonClick() {
                this.deleteFacebookGroupReference().then(
                    (success) => {
                        this.showDataUploadSuccessMessage();
